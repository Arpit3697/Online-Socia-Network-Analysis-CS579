...
def bfs(graph, root, max_depth):
    ...
    queue.append(root)
    cnt = 0
    node2distances[root]=cnt
    node2num_paths[root]=cnt+1
    node_seen = [root]
    while queue:
        node_Pop = queue.popleft()
        node_Neighbour= sorted(graph.neighbors(node_Pop))
        cnt = node2distances[node_Pop]
        if cnt >= max_depth:
            break;
        for node in node_Neighbour:
            if node not in node_seen:
                ...
                queue.append(node)
                node2parents[node] = [node_Pop]
                node2distances[node] = cnt + 1
                node2num_paths[node] = node2num_paths[node_Pop]
            else:
                last_cnt = node2distances[node]
                current_cnt = cnt + 1
                if(last_cnt==current_cnt):
                    node2num_paths[node] = node2num_paths[node]+node2num_paths[node_Pop]
                    node2parents[node].append(node_Pop)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    for node in sorted(node2distances, key=node2distances.get, reverse=True):
        if node != root:
            node_length = node2num_paths[node]
            if node_length > 1:
                for parent_Node in node2parents[node]:
                    final_Tuple = tuple(sorted([node, parent_Node]))
                    edge_Bet[final_Tuple]=(float)((count_Node[node]+1)/len(node2parents[node]))
                    count_Node[parent_Node]=(float)(count_Node[parent_Node]+edge_Bet[final_Tuple])
            else:
                parent_Node = node2parents[node][0]
                final_Tuple = tuple(sorted([node,parent_Node]))
                edge_Bet[final_Tuple] = float(count_Node[node]+1)
                count_Node[parent_Node] = float(count_Node[parent_Node] + edge_Bet[final_Tuple])
    return edge_Bet
    pass
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in graph.nodes():
        ...
        bets = bottom_up(node, node2distances, node2num_paths, node2parents)
        for edge in bets:
            if edge in approx_betweenness:
                approx_betweenness[edge] = approx_betweenness[edge] + bets[edge]
            else:
                approx_betweenness[edge] = bets[edge]
    for i,j in approx_betweenness.items():
        approx_betweenness[i]=j/2
...
def partition_girvan_newman(graph, max_depth):
    ...
    approx_betweenness = approximate_betweenness(graph_Copy,max_depth)
    rslt=sorted(approx_betweenness.items(),key=lambda a: (-a[1], a[0]))
    cmpnt = get_components(graph_Copy)
    while (True):
        graph_Copy.remove_edge(*(rslt[index][0]))
        cmpnt = get_components(graph_Copy)
        if len(cmpnt) > 1:
            break
        elif len(cmpnt) == 1:
...
def get_subgraph(graph, min_degree):
    ...
    for i in graph.nodes():
        if graph.degree(i)>= min_degree:
            Output_graph.add(i);
    return graph.subgraph(Output_graph)
    pass
...
def volume(nodes, graph):
    ...
    for edge in graph.edges():
        if(((edge[0] in nodes) or (edge[1] in nodes)) and (edge not in explrd)):
                explrd.append(edge)
    return len(explrd)
    pass
...
def norm_cut(S, T, graph):
    ...
    """
    ###TODO
    cut_graph = cut(S, T, graph)
    return ((float(cut_graph) / volume(S, graph)) + (float(cut_graph) / volume(T, graph)))
    pass
...
def brute_force_norm_cut(graph, max_size):
    ...
    for i in range(1,max_size+1):
        for first_Cut in combinations(graph.edges(), i):
            graph_Copy.remove_edges_from(first_Cut)
            num_cmpnts = get_components(graph_Copy)
            if len(num_cmpnts) == 2:
                normal_Cut = norm_cut(num_cmpnts[0], num_cmpnts[1], graph)
                list_Edges.append((normal_Cut,list(first_Cut)))
            graph_Copy.add_edges_from(first_Cut)
    return list_Edges
    pass
...
def score_max_depths(graph, max_depths):
    ...
    List_Maxcut = []
    for i in max_depths:
        cmpnts = partition_girvan_newman(graph,i);
        if(len(cmpnts)>1):
            normal_cut = norm_cut(cmpnts[0],cmpnts[1],graph);
            List_Maxcut.append((i,normal_cut));
    return List_Maxcut;
    pass
...
def make_training_graph(graph, test_node, n):
    ...
    list_Nodes=sorted(graph_Tmp.neighbors(test_node))
    for i in list_Nodes[0:n]:
            graph_Tmp.remove_edge(*(test_node,i))
    return  graph_Tmp
    pass
...
def jaccard(graph, node, k):
    ...
    for node_item in graph.nodes():
        if node_item != node and not graph.has_edge(node, node_item):
            new_neighbours = set(graph.neighbors(node_item))
            jaccard = len(neighbors.intersection(new_neighbours))/len(neighbors.union(new_neighbours))
            list_Edges.append(((node,node_item),jaccard))
    return sorted(list_Edges,key=lambda a: -a[1])[0:k]
    pass
...
def evaluate(predicted_edges, graph):
    ...
    for edge in range(len(predicted_edges)):
        if(graph.has_edge(*(predicted_edges[edge]))):
            fraction += 1
    return float(fraction/len(predicted_edges))
    pass
