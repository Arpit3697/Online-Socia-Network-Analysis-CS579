...
def featurize(movies):
    ...
    for i in movies['tokens']:
        term_count.update(set(i))
    vocab = {term : num for num, term in enumerate(sorted(term_count))}
    # modify movies
    column_numbers = len(vocab)
    matrix_list = []
    N = len(movies)
    for i in movies['tokens']:
        term_count1 = Counter(i)
        total = sum(term_count1.values())
        max_k = max(term_count1.values())
        tfidf = []
        for term, tf in term_count1.items():
            df = term_count[term]
            tfidf.append(tf / max_k * math.log10(N / df))
        row = [0] * len(term_count1)
        column = []
        for i in term_count1:
            column.append(vocab[i])
        matrix_list.append(csr_matrix((tfidf, (row, column)), shape=(1, column_numbers)))
    movies['features'] = matrix_list
    return(movies, vocab)
    pass
...
def make_predictions(movies, ratings_train, ratings_test):
    ...
    for i, row in ratings_test.iterrows():
        ...
        cos_list = []
        cos_sum = 0
        for j, row1 in train_movie.iterrows():
            tFeature = movies.loc[movies['movieId'] == row1['movieId']].squeeze()['features']
            cos_sim = cosine_sim(iFeature, tFeature)
            if cos_sim > 0:
                cos_list.append(cos_sim * row1['rating']);
                cos_sum += cos_sim
        if cos_sum > 0:
            result_list.append(sum(cos_list) / cos_sum)
        else:
            result_list.append(train_movie['rating'].mean()) 
